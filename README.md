...
Experiment specs are frozen under `experiments/<name>/spec.yaml` before anything runs; every
run they start records `experiment = {role, stage, spec_path, spec_sha256}` in its manifest
(v5) and refuses a dirty working tree. Derived tables live under `data/<name>/` and the report
under `docs/experiments/`; both are regenerated byte-identically from `runs/` (raw
trajectories never enter the repository).
